...
// import { LinearGradient } from 'expo-linear-gradient'; // Removed to debug loading issue
...
export default function HomeScreen() {
    ...
    useEffect(() => {
        ...
        AsyncStorage.getItem('userName').then(name => name && setCurrentUserName(name));
        ...
        setTimeout(async () => {
            try {
                ...
                const resp = await fetch(`${API_URL}/api/restaurants/donate`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ restaurantId: selectedRestaurant.id, amount, donorId: currentUserId || 'anon' })
                });
    ...
    const handleRegisterPartner = async () => {
        ...
        try {
            const resp = await fetch(`${API_URL}/api/restaurants/register`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ...partnerForm, initialFunds: Number(partnerForm.initialFunds) || 0, ownerId: currentUserId || 'partner' })
            });
        ...
        <SafeAreaView style={styles.container}>
            ...
            <View style={[styles.header, { backgroundColor: '#fff' }]}>
                ...
                <View style={styles.identityBadge}>
                    ...
                    <View>
                        <Text style={styles.identitySubtitle}>ACTIVE CITIZEN</Text>
                        <Text style={styles.identityName}>{currentUserName || 'Anonymous'}</Text>
                    </View>
                </View>
            </View>
...
const styles = StyleSheet.create({
    ...
    identityBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 8, borderRadius: 100, borderWidth: 1, borderColor: '#f3f4f6', shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 5 },
    ...
    switcherBg: { flexDirection: 'row', backgroundColor: '#eff6ff', padding: 4, borderRadius: 14, backgroundColor: '#f1f5f9' },
    switchBtn: { flex: 1, paddingVertical: 10, alignItems: 'center', borderRadius: 11 },
    switchBtnActive: { backgroundColor: '#fff', shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 4, elevation: 2 },
    switchText: { fontSize: 12, fontWeight: '700', color: '#94a3b8' },
    switchTextActive: { color: '#059669' },

    actionRow: { paddingHorizontal: 16, marginBottom: 10 },
    secondaryAction: { flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start', borderBottomWidth: 1, borderBottomColor: '#059669', paddingBottom: 2 },
    secondaryActionText: { color: '#059669', fontSize: 13, fontWeight: '600', marginLeft: 6 },

    card: { backgroundColor: '#fff', borderRadius: 24, padding: 20, marginBottom: 16, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 10, elevation: 4, marginHorizontal: 2 },
    cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 },
    restaurantName: { fontSize: 20, fontWeight: '800', color: '#111827', flex: 1 },
    badge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8 },
    badgeText: { fontSize: 10, fontWeight: '800' },
    statsRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f9fafb', borderRadius: 16, padding: 15, marginBottom: 20 },
    statItem: { flex: 1, alignItems: 'center' },
    statLabel: { fontSize: 10, fontWeight: '800', color: '#94a3b8', marginBottom: 5 },
    statValue: { fontSize: 22, fontWeight: '900', color: '#374151' },
    divider: { width: 1, backgroundColor: '#e2e8f0' },
    actionButton: { height: 55, borderRadius: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
